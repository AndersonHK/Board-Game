...
83. Edition 0 card rendering is now a front-only PNG pipeline under [assets/scripts](../assets/scripts): generate reusable frames, render individual card fronts from `defines/cards`, then build letter-size front atlases from the generated fronts. Final AI art should be moved into `assets/card-art/final/<deck-slug>/<card-id>.png`, with [assets/card-art/AI_ART_HANDOFF.md](../assets/card-art/AI_ART_HANDOFF.md) as the prompt and destination checklist.
